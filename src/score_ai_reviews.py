"""Score the 1,000 Gen-Review AI reviews using Claude via Bedrock (neutral prompt)."""
...
def main():
    ...
    print(f"AI reviews to score: {len(ai_indices)}")

    # Check for checkpoint
    ckpt_path = DATA_DIR / "ai_scores_checkpoint.json"
    completed = {}
    if ckpt_path.exists():
    ...
    remaining = [i for i in ai_indices if i not in completed]
    ...
    if not remaining:
        ...
        pbar = tqdm(total=len(work), desc="Scoring AI reviews via Bedrock")
        newly_done = 0
    ...
    print(f"\nApplied LLM scores to {applied} AI reviews")
    print(f"Saved to features_cache.csv")

    # Show stats
    ai = df[df["label"] == 1]
    print(f"\nAI review marker means (LLM-scored):")
    for m in MARKER_NAMES:
